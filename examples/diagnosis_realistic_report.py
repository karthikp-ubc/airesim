...
def verdict(data, rows):
    ...
    if any_benefit:
        ...
        lines.append(
            ...
            "re-triggered only once standbys run out. FFF can only bench a bad server "
            "that has already failed at least once -- one that hasn't yet failed looks "
            "identical to a good one. At these parameters headroom is "
            ...
            "the entire initial bad population (~368), so a full selection can bench "
            "every *known*-bad server (not the whole population outright), and "
            "FewestFailuresFirst measurably benches them (faulty-in-active time-average "
            "129.7 vs Random's 170.9, a 24% reduction, saving ~272 h) -- confirming the "
            "mechanism works when headroom allows it, and that its absence here, "
            "not rare consultation, is why FFF and FAFF show "
